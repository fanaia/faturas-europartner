...
async function receiveWebhook({ empresaCode, token, payload }) {
  ...
  const [configuration, expectedToken, credentials] = await Promise.all([
    getCentralConfiguration(),
    getCompanyWebhookToken(empresa),
    getCompanyCredentials(empresa),
  ]);
  rateLimit(empresaCode, Number(configuration.webhookRateLimitPerMinute || 120));
